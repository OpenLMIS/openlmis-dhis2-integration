...
&& (docker run --rm --network ${COMPOSE_PROJECT_NAME_LOWER_CASE//.}_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:6.1.0 -t pgsql -host db -port 5432 -db open_lmis -s dhis2-integration -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 90 \
...
&& zip -r $WORKSPACE/erd/erd-dhis2-integration.zip . \
&& popd \
&& rmdir output \
&& rm .env \
